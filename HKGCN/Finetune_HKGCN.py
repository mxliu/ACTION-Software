...
#from GCN_Transformer import Transformer
...
parser.add_argument('--threshold', default=0.5, type=float, 
                    help='thredshold of the acc prediction')
...
parser.add_argument('--encoder_type',
                    choices=['HKGCN','HKGAT','GCN', 'GIN', 'GAT','BrainNetCNN','Transformer'],
                    default='HKGCN',  
...
def calculate_metric(gt, pred, threshold=args.threshold):
    """
...
class Model(nn.Module):
    ...
    def __init__(self, dim, pred_dim, c, a,a_couping, a_fMRI, a_DTI, weight_fMRI, weight_DTI,dropout, pretrained_path=None):
        """
        ...
        self.c = c
        self.a = a
        self.encoder = HGNNEncoder(116, dim, c, a_couping, a_DTI, a_fMRI, weight_fMRI,weight_DTI, dropout, pretrained_path)
    ...
    def forward(self, DTI,adj_DTI, fMRI):
        ...
        fea_coupled, adj_coupled, DTI_HKGCN,fMRI_HKGCN,adj_DTI,adj_fMRI   = self.encoder(DTI,adj_DTI, fMRI) 
        #print("fea_coupled.shape",fea_coupled.shape)      
        z = reduce(fea_coupled, 'b n c ->b c', 'mean')  # z is tensor(8,64), which is graph representation generated by GNN

        p = self.predictor(z)  # NxC
        output = F.softmax(p, dim=1)  #

        return p, z, output, fea_coupled, DTI_HKGCN,fMRI_HKGCN
...
for seed in [ 1,2,3,4,5]:
    ...
    #保存输出结果
    result_dir_csv = save_path + "/HAND" +save_path1 + str(time_str) + ".csv"
    ...
    #训练和评估模型
    # 获取标签数据
    y_labels = full_dataset.y_DTI.numpy()  # 你的类别标签 (0,1)

    for fold, (train_idx, test_idx) in enumerate(kfold.split(full_dataset.data_DTI, y_labels)):
        ...
        pretrained='Pretrained_HKGCN_019.pth.tar'.format(args.encoder_type) 
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        print(f'DEVICE: {device}')
        model = Model(args.dim, args.pred_dim,args.c, args.a, args.a_coupling, args.a_DTI, args.a_fMRI, args.weight_fMRI, args.weight_DTI, args.dropout, pretrained_path = pretrained)  # args.pred_dim
            #def __init__(self, dim, pred_dim,c,a, num_layer, nhead):
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            for i, data in enumerate(training_data_loader):
                ...
                p, z, output, fea_coupled, DTI_HKGCN, fMRI_HKGCN = model(data_DTI, adj_DTI, bold)
            ...
            with torch.no_grad():
                for i, data in enumerate(testing_data_loader):
                    ...
                    All_DTI_HKGCN.append(DTI_HKGCN.cpu())
                    All_fMRI_HKGCN.append(fMRI_HKGCN.cpu())
                ...
                All_DTI_HKGCN = torch.cat(All_DTI_HKGCN, dim=0)
                All_fMRI_HKGCN = torch.cat(All_fMRI_HKGCN, dim=0)
                ...
                if epoch == args.epochs - 1:
                    ...
                    final_threshold = find_optimal_threshold(y_true, PPre_score, threshold_scale = args.threshold_scale)

                    y_pred = (PPre_score > final_threshold).int()
                    final_model_info = {
                        "epoch": epoch + 1,
                        "final_threshold": final_threshold,
                        "final_model_path": model_final_path
                    }
                    final_model_info_serializable = convert_to_serializable(final_model_info)

                    with open(os.path.join(final_fold_dir, "final_model_info.json"), "w") as f:
                        json.dump(final_model_info_serializable, f)

                    torch.save(model_final_state, model_final_path)  # 保存模型
                    # 保存最终模型文件名信息
                    ...
                    final_results = {
                        ...
                        "DTI_HKGCN": All_DTI_HKGCN.cpu(),
                        "fMRI_HKGCN": All_fMRI_HKGCN.cpu(),
                        "p":All_p.cpu(),
                        "z":All_z.cpu(),
                        "fea_coupled":All_fea_coupled.cpu(),
                        "output":All_output.cpu(),  #output是最终结果
                    }
                    
                    for key in ["index", "DTI_initial", "fMRI_initial", "y_true", "y_pred",  "DTI_HKGCN", "fMRI_HKGCN","p","z","fea_coupled", "output"]:
                ...
                # 计算最优阈值
                optimal_threshold = find_optimal_threshold(y_true, PPre_score)
                y_pred = (PPre_score > optimal_threshold).int()
        



            with open(os.path.join(final_fold_dir, "split_indices_validation.txt"), "w") as f:
                f.write(f"Train indices: {train_idx}\n")
                f.write(f"Test indices: {test_idx}\n")
            final_acc, final_sen, final_spe, final_bac, final_pre, final_f1_score = calculate_metric(y_true, y_pred,args.threshold)
                
        print(f"New final model saved for fold {fold} at epoch {epoch + 1 } with accuracy {final_acc:.4f}")
        with open(result_dir_csv, 'a') as f:
            f.write(f"New final model saved for fold {fold} at epoch {epoch + 1} with accuracy {final_acc:.4f}" )
        
        for key, value in final_results.items():
            torch.save(value, os.path.join(final_fold_dir, f"{key}_final.pt"))

        print('PPre_score=',PPre_score,'threshold', args.threshold)
        print('y_pred=',y_pred)

        acc, sen, spe, bac, pre, f1_score = calculate_metric(y_true, y_pred, threshold=args.threshold)
