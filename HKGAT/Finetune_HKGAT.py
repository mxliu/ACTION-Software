...
class Model(nn.Module):
    ...
    def __init__(self, dim, pred_dim, c, a, pretrained_path=None):
        """
        ...
        self.encoder = Module_1(116, dim, c, a, pretrained_path)
    ...
    def forward(self, fMRI):
        ...
        fea   = self.encoder( fMRI) 
        z = reduce(fea, 'b n c ->b c', 'mean') 

        p = self.predictor(z)  
        output = F.softmax(p, dim=1)  
        return z, p, output, fea
...
for fold, (train_idx, test_idx) in enumerate(kfold.split(full_dataset)):
    ...
    model =Model(args.dim, args.pred_dim,args.c, args.a, pretrained_path = pretrained)  # args.pred_dim
